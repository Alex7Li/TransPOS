...
def train_epoch(
    ...
    for i in tqdm(range(n_iters),
                  desc='Epoch progress', mininterval=5):
...
def validation_acc(model: MapperModel, val_dataloader) -> float:
    model.eval()
    # TODO
    return 0

def train_model(model: MapperModel, y_dataloader, z_dataloader, 
        val_dataloader, n_epochs, save_path):
    optimizer = torch.optim.NAdam(model.parameters(), lr=3e-5, weight_decay=1e-4)
    scheduler = get_scheduler(
        name="linear", optimizer=optimizer, num_warmup_steps=0, num_training_steps=
            min(len(y_dataloader), len(z_dataloader))*n_epochs
    )
    best_validation_acc = 0
    valid_acc = 0
    for epoch_index in tqdm(range(0, n_epochs),
                  desc='Training epochs', maxinterval=50):
        train_loss = train_epoch(y_dataloader, z_dataloader, model, optimizer, scheduler)
        if val_dataloader is not None:
            valid_acc = validation_acc(model, val_dataloader)
...
def main(y_dataset_name, z_dataset_name, model_name):
    batch_size = 32
    n_epochs = 5
    save_path = Path('models') / (model_name.split('/')[-1] + "_mapper_" + y_dataset_name + "_" + z_dataset_name)
    y_dataset = training.get_dataset(y_dataset_name, 'all')
    z_dataset = training.get_dataset(z_dataset_name, 'all')
    y_dataloader = training.get_dataloader(model_name, y_dataset, batch_size, shuffle=True)
    z_dataloader = training.get_dataloader(model_name, z_dataset, batch_size, shuffle=True)
    model = MapperModel('vinai/bertweet-large', y_dataset.num_labels, z_dataset.num_labels)
    val_dataloader = None
    if y_dataset == 'tweebank' and z_dataset == 'ark':
        val_dataloader = None # TODO
        pass
    train_model(model, y_dataloader, z_dataloader, val_dataloader, n_epochs, save_path)
