...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, encoder_hidden_dim):
        ...
        self.encoderDropout = 0.2
        self.embedding = nn.Sequential(
            nn.Linear(input_dim, encoder_hidden_dim // 2),
            nn.BatchNorm1d(encoder_hidden_dim // 2),
            nn.GELU(),
            nn.Dropout(self.encoderDropout),
...
DecoderOutput = smallDecoderyz.forward(
    smallEncoder.forward(vec_dataset[0][0]),
    ybar=smallDecoderzy.forward(smallEncoder.forward(vec_dataset[0][0])),
)
...
print(
    "test decoder shape: ",
    smallDecoderyz.forward(smallEncoder.forward(vec_dataset[0][0])).shape,
)
DecoderOutput = smallDecoderzy.forward(
    smallEncoder.forward(vec_dataset[0][0]),
    zbar=smallDecoderyz.forward(smallEncoder.forward(vec_dataset[0][0])),
)
...
class Label2Label(nn.Module):
    ...
    def __init__(
        self, input_dim, encoder_hidden_dim, decoder_hidden_dim, y_classes, z_classes
    ):
        super(Label2Label, self).__init__()
        self.encoder = Encoder(input_dim, encoder_hidden_dim)
        self.decoderYZ = Decoder_YtoZ(
            encoder_hidden_dim, decoder_hidden_dim, y_classes, z_classes
        )
        self.decoderZY = Decoder_ZtoY(
            encoder_hidden_dim, decoder_hidden_dim, y_classes, z_classes
        )
...
input_dim = 768
Label2LabelModel = Label2Label(
    input_dim, hidden_dim, decoder_hidden_dim, y_classes, z_classes
)
