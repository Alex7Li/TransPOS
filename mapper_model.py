...
from mapper_helper import hardToSoftLabel
...
class SimpleRNN(torch.nn.Module):
    def __init__(self, embed_dim, hidden_dim, n_layers):
        super().__init__()
        w = torch.empty(2 * n_layers, hidden_dim, dtype=torch.float32)
        torch.nn.init.kaiming_normal_(w)
        self.rnn_init = torch.nn.Parameter(w)
        self.register_parameter(name="rnn_init", param=self.rnn_init)
        self.RNN = torch.nn.GRU(
            embed_dim, hidden_dim, num_layers=n_layers, batch_first=True, bidirectional=True
        )
    def forward(self, x):
        rnn_init = torch.stack([self.rnn_init for _ in range(x.shape[0])], dim=1)
        # rnn_init: [n_layers * 2, B, rnn_hidden]
        x, _ = self.RNN.forward(x, rnn_init)
        return x
...
class Label2LabelDecoder(torch.nn.Module):
    def __init__(
        ...
        torch.nn.init.kaiming_normal_(w)
        self.yRNN = SimpleRNN(y_embed_dim, rnn_hidden, n_rnn_layers)
        xy_hidden = 512
...
class MapperModel(torch.nn.Module):
    def __init__(self, base_transformer_name: str, n_y_labels: int, n_z_labels: int, parameters):
        ...
        self.yz_distinguisher = nn.Sequential(
            SimpleRNN(n_y_labels + n_z_labels, 256, 3),
            nn.Linear(256 * 2, 1),
        )
        ...
        self.auxilary_params = itertools.chain(
            self.yz_distinguisher.parameters(),
    ...
    def p_originally_z(self, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        """Given a true label y and predicted label z
        OR
        a predicted label y and a true label z
        
        This function will predict which one is real: sigmoid(p_originally_z(y, z))
        should give the probablity of z being real.
        """
        y = hardToSoftLabel(y, self.n_y_labels)
        z = hardToSoftLabel(z, self.n_z_labels)
        yz = torch.cat([y, z], dim=2)
        d = self.yz_distinguisher(yz)
        return torch.sigmoid(torch.mean(d, dim=1).flatten())
