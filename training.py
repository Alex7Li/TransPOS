...
def train_epoch_aug(model, train_dataloader, optimizer, scheduler):
    ...
    for batch in tqdm(train_dataloader, desc="Training"):
        ...
        batch_input_ids, batch_input_ids_aug, mid = generate_mask_and_data(
            batch["input_ids"], "input", mid=None
        )

        batch_attention_mask, batch_attention_mask_aug, _ = generate_mask_and_data(
            batch["attention_mask"], "attention", mid=mid
        )
        batch_labels, batch_labels_aug, _ = generate_mask_and_data(
            batch["labels"], "labels", mid=mid
        )
        ...
        batch_aug = {
            "input_ids": batch_input_ids_aug,
            "attention_mask": batch_attention_mask_aug,
            "labels": batch_labels_aug,
        }
        ...
        std_loss = torch.mean(nn.functional.relu(1 - std_z_a)) + torch.mean(
        ...
        cov_z_a = (
            torch.bmm(z_aug, torch.transpose(z_aug, 1, 2))
            / batch_normal["input_ids"].shape[0]
        )
        cov_z_b = (
            torch.bmm(z_normal, torch.transpose(z_normal, 1, 2))
            / batch_normal["input_ids"].shape[0]
        )
        cov_loss = (torch.sum(cov_z_a) - torch.diagonal(cov_z_a, 0).pow(2).sum()) + (
            torch.sum(cov_z_b) - torch.diagonal(cov_z_b, 0).pow(2).sum()
        )
...
def get_dataloader(model_name: str, dataset, batch_size, shuffle=False):
    tokenizer = AutoTokenizer.from_pretrained(
        model_name, add_prefix_space=True, use_fast=True, model_max_length=512
    )
    if model_name == "gpt2":
...
def training_loop_aug(
    model, train_dataloader, val_dataloader, dataset_name, n_epochs, save_path
):
    ...
    for i in tqdm(range(0, n_epochs), desc="Training epochs"):
        ...
        val_acc_normal = get_validation_acc(
            preds_normal, labels, dataset_name, dataset_name
        )
...
def pipeline(hparams, run_aug=False):
    ...
    if run_aug:
        ...
        val_dataloader = get_augmented_dataloader(
            dataset=hparams["dataset"], partition="dev", model="gpt2"
        )  # added
        for batch in val_dataloader:
            batch = {k: v.to(device) for k, v in batch.items()}
        return training_loop_aug(
            model,
            train_dataloader,
            val_dataloader,
            hparams["dataset"],
    ...
    for model_name in model_names:
        ...
        for train_dataset_name in dataset_names:
            ...
            for test_dataset_name in dataset_names:
                print(
                    f"Validating: {test_dataset_name}, with model: {model_name}, trained on: {train_dataset_name}"
                )
                ...
                acc = get_validation_acc(
                    preds, labels, train_dataset_name, test_dataset_name
                )
                print(f"Test Accuracy on {test_dataset_name}: {round(100*acc,3)}%")
                result_dict[model_name][train_dataset_name][test_dataset_name] = (
                    100 * acc
                )
                torch.cuda.empty_cache()
