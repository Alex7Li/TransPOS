...
class ArkAugDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(batch):
        ...
        batch_x = [
            torch.as_tensor(x, dtype=torch.float64, device=device) for x, y in batch
        ]
        # batch_xhat = [torch.as_tensor(xhat,dtype=torch.float64,device=device) for x,y in batch]
        batch_Y = [
            torch.as_tensor(y, dtype=torch.long, device=device) for x, y in batch
        ]
...
class TPANNAugDataset(torch.utils.data.Dataset):
    def __init__(self, X, Y):
        ...
        self.TPANN_POS_INDEX_MAPPING, self.TPANN_INDEX_POS_MAPPING = create_pos_mapping(
            TPANN_POS_TAGS
        )
        self.Y = []
...
def generate_mask_and_data(x_input, input_type, mid=None):
    ...
    batch_x_n = (
        torch.tensor(batch_x_n).cuda()
        if torch.cuda.is_available()
        else torch.tensor(batch_x_n)
    )
    batch_xhat = (
        torch.tensor(batch_xhat).cuda()
        if torch.cuda.is_available()
        else torch.tensor(batch_xhat)
    )
...
def get_dataloader(model_name, dataset, batch_size, shuffle=False):
    ...
    dataloader = DataLoader(
        compat_dataset, shuffle=shuffle, batch_size=batch_size, collate_fn=data_collator
    )
    return dataloader
            ...
            if train_Y[i][j] in [
                ...
                for s, synset in enumerate(wn.synsets(train_X[i][j])):
                    if s == 0:
                        continue
                    if (
                        synset.pos() == train_Y[i][j].lower()
                        or synset.pos() == temp_pos.lower()
                    ):
...
def get_augmented_dataloader(dataset="ark", partition="train", model="gpt2"):
    ...
    TPANN_POS_INDEX_MAPPING, TPANN_INDEX_POS_MAPPING = create_pos_mapping(
        TPANN_POS_TAGS
    )
    if dataset.lower() == "ark":
        ark_train_x, ark_train_y = load_ark(return_lists=True, partition=partition)
        ark_aug_train_x, ark_aug_train_y = get_augmented_dataset(
            ark_train_x, ark_train_y
        )
    ...
    if dataset.lower() == "tpann":
        ...
        tpann_aug_train_x, tpann_aug_train_y = get_augmented_dataset(
            tpann_train_x, tpann_train_y
        )
    ...
    if dataset.lower() == "atis":
        atis_train_x, atis_train_y = data_reader(
            f"AtisDataset/en_atis-ud-{partition}.conllu"
        )
        atis_aug_train_x, atis_aug_train_y = get_augmented_dataset(
            atis_train_x, atis_train_y
        )
    ...
    if dataset.lower() == "gum":
        gum_train_x, gum_train_y = data_reader(
            f"GUMDataset/en_gum-ud-{partition}.conllu"
        )
        gum_aug_train_x, gum_aug_train_y = get_augmented_dataset(
            gum_train_x, gum_train_y
        )
    ...
    if dataset.lower() == "tweebank":
        twee_train_x, twee_train_y = data_reader(
            f"TweeBankDataset/Tweebank-dev/en-ud-tweet-{partition}.conllu"
        )
        twee_aug_train_x, twee_aug_train_y = get_augmented_dataset(
            twee_train_x, twee_train_y
        )
...
if __name__ == "__main__":
    ...
    if test_tpann_aug:
        ...
        tpann_aug_train = TPANNAugDataset(tpann_aug_train_x, tpann_aug_train_y)
        tpann_aug_dataloader = get_dataloader(
            "gpt2", tpann_aug_train, 20, shuffle=False
        )
    ...
    if test_twee:
        ...
        twee_train_x, twee_train_y = data_reader(
            "TweeBankDataset/Tweebank-dev/en-ud-tweet-train.conllu"
        )
        twee_aug_train_x, twee_aug_train_y = get_augmented_dataset(
            twee_train_x, twee_train_y
        )
